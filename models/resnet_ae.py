...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, configs, final_dim, bottleneck=False, final_fc=True):
        ...
        if bottleneck:
            ...
            self.conv5 = EncoderResidualBlock(in_channels=256, hidden_channels=final_dim, layers=configs[3], downsample_method="conv")

        if final_fc:
            # self.fc = nn.Linear(final_dim*7*7, final_dim, bias=False) # 224x224
            self.fc = nn.Linear(final_dim*2*2, final_dim) # 64x64
...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self, configs, final_dim, bottleneck=False, initial_fc=True):
        ...
        if initial_fc:
            self.fc = nn.Sequential(
                # nn.Linear(final_dim, final_dim*7*7), # 224x224
                # nn.Unflatten(1, (final_dim,7,7)) # 224x224

                nn.Linear(final_dim, final_dim*2*2), # 64x64
                nn.Unflatten(1, (final_dim,2,2)) # 64x64
            )
        else:
            self.fc = nn.Unflatten(1, (final_dim,7,7))
        ...
        if bottleneck:
            ...
            self.conv1 = DecoderResidualBlock(hidden_channels=final_dim, output_channels=256, layers=configs[0])
